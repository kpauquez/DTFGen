...
const HMR_CATS = ["sujeto", "pose", "accion", "ambiente", "angulo", "estilo", "iluminacion", "atmosfera", "detalles", "calidad"];
...
const buildPrompt = (sel, custom, negativo, hmr, textos) => {
  const get = (cat) => (custom[cat] && custom[cat].trim()) ? custom[cat].trim() : (sel[cat] || null);
  const formatoVal = get("formato");
  ...
  const parts = [
    ...
    // 10. Detalles Extra
    h("detalles"),
    // 11. Paleta + Fondo + Máscara
    get("paleta") ? `paleta ${get("paleta").toLowerCase()}` : null,
    (() => {
      const fondoVal = custom.fondo && custom.fondo.trim()
        ? custom.fondo.trim()
        : Array.isArray(sel.fondo) && sel.fondo.length > 0
          ? sel.fondo.map(f => f.toLowerCase()).join(", ")
          : null;
      return fondoVal ? `fondo ${fondoVal}` : null;
    })(),
    (() => {
      const mascaraArr = Array.isArray(sel.mascara) ? sel.mascara : (sel.mascara ? [sel.mascara] : []);
      if (!mascaraArr.length) return null;
      const descs = mascaraArr.map(v => MASCARA_OPTIONS.find(m => m.value === v)?.desc || v).join(", ");
      return `layer mask: ${descs}`;
    })(),
    // 13. Formato
    formatoVal ? (FORMATO_MAP[formatoVal] || `formato ${formatoVal.toLowerCase()}`) : null,
    // 14. Calidad SD (tarjeta HMR) + Calidad módulo
    h("calidad"),
            ...
            <div key={cat} style={{ background: "#111", border: `1.5px solid ${card.locked ? "#3a3a1a" : "#1e1e1e"}`, borderRadius: "11px", padding: "11px", position: "relative", transition: "all 0.25s ease", animation: "fadeIn 0.25s ease" }}>
              ...
              >
                {card.analyzingImg
                  ...
                  : <>
                      <p style={{ color: card.locked ? "#b8a840" : "#bbb", fontSize: "12px", lineHeight: "1.5", margin: 0 }}>{card.value || <span style={{ color: "#444" }}>—</span>}</p>
...
const ModuleRow = ({ cat, selected, onToggle, custom, onCustomChange }) => {
  const isCustomActive = !!(custom[cat] && custom[cat].trim().length > 0);
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", gap: "12px", flexWrap: "wrap" }}>
        <span style={{ color: "#bbb", fontWeight: "600", fontSize: "13px", minWidth: "76px" }}>{LABELS[cat]}:</span>
        <div style={{ display: "flex", flexWrap: "wrap", gap: "7px", flex: 1 }}>
          {OPTIONS[cat].map((v) => (
            <Toggle key={v} label={v}
              active={!isCustomActive && selected[cat] === v}
              disabled={isCustomActive}
              onClick={() => { if (!isCustomActive) onToggle(cat, v); }}
              color={cat === "paleta" ? PALETTE_COLORS[v] : undefined}
            />
          ))}
        </div>
      </div>
      <div style={{ display: "flex", alignItems: "center", gap: "8px", marginTop: "8px", paddingLeft: "88px" }}>
        <span style={{ fontSize: "11px", color: "#3a3a3a", fontWeight: "500", whiteSpace: "nowrap", minWidth: "60px" }}>✎ Agregar:</span>
        <input value={custom[cat] || ""} onChange={(e) => onCustomChange(cat, e.target.value)}
          onKeyDown={(e) => {
            if (e.key === "Enter" && custom[cat]?.trim()) {
              const val = custom[cat].trim();
              if (!OPTIONS[cat].includes(val)) OPTIONS[cat] = [...OPTIONS[cat], val];
              onToggle(cat, val);
              onCustomChange(cat, "");
            }
          }}
          ...
          style={{
            ...
            color: "#555", fontFamily: "'DM Sans', sans-serif", fontSize: "12px", outline: "none", transition: "all 0.18s",
          }}
          onFocus={(e) => { e.target.style.borderColor = "#4caf5099"; e.target.style.color = "#e0e0e0"; }}
          onBlur={(e) => { e.target.style.borderColor = "#252525"; e.target.style.color = "#555"; }}
        />
        <button
          onClick={() => {
            const val = custom[cat]?.trim();
          ...
          style={{ padding: "6px 12px", background: custom[cat]?.trim() ? "#1a2a1a" : "#181818", border: `1.5px solid ${custom[cat]?.trim() ? "#4caf5066" : "#252525"}`, borderRadius: "8px", color: custom[cat]?.trim() ? "#4caf50" : "#444", fontFamily: "'DM Sans', sans-serif", fontWeight: "600", fontSize: "12px", cursor: custom[cat]?.trim() ? "pointer" : "not-allowed", whiteSpace: "nowrap", transition: "all 0.15s" }}
          onMouseOver={(e) => { if (custom[cat]?.trim()) { e.currentTarget.style.borderColor = "#4caf50"; e.currentTarget.style.background = "#1e3a1e"; }}}
          onMouseOut={(e) => { if (custom[cat]?.trim()) { e.currentTarget.style.borderColor = "#4caf5066"; e.currentTarget.style.background = "#1a2a1a"; }}}
        >+ Agregar</button>
      </div>
    </div>
  );
...
const FondoRow = ({ selected, onToggle, custom, onCustomChange }) => {
  const isCustomActive = !!(custom.fondo && custom.fondo.trim().length > 0);
  const mascaraArr = Array.isArray(selected.mascara) ? selected.mascara : (selected.mascara ? [selected.mascara] : []);
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", gap: "12px", flexWrap: "wrap" }}>
        <span style={{ color: "#bbb", fontWeight: "600", fontSize: "13px", minWidth: "76px" }}>Fondo:</span>
        <div style={{ display: "flex", flexWrap: "wrap", gap: "7px", flex: 1 }}>
          {OPTIONS.fondo.map((v) => {
            const fondoArr = Array.isArray(selected.fondo) ? selected.fondo : (selected.fondo ? [selected.fondo] : []);
            return (
              <Toggle key={v} label={v}
                active={!isCustomActive && fondoArr.includes(v)}
                disabled={isCustomActive}
                onClick={() => { if (!isCustomActive) onToggle("fondo", v); }}
              />
            );
          })}
        </div>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "8px", marginTop: "8px", paddingLeft: "88px" }}>
<span style={{ fontSize: "11px", color: "#3a3a3a", fontWeight: "500", whiteSpace: "nowrap", minWidth: "60px" }}>✎ Agregar:</span>
        <input value={custom.fondo || ""} onChange={(e) => onCustomChange("fondo", e.target.value)}
          onKeyDown={(e) => {
            if (e.key === "Enter" && custom.fondo?.trim()) {
              const val = custom.fondo.trim();
              if (!OPTIONS.fondo.includes(val)) OPTIONS.fondo = [...OPTIONS.fondo, val];
              onToggle("fondo", val);
              onCustomChange("fondo", "");
            }
          }}
          placeholder="Ej: degradado azul, humo blanco..."
          style={{
            flex: 1, background: "#181818", border: "1.5px solid #252525",
            borderRadius: "8px", padding: "7px 12px", color: "#555",
            fontFamily: "'DM Sans', sans-serif", fontSize: "12px", outline: "none", transition: "all 0.18s",
          }}
          onFocus={(e) => { e.target.style.borderColor = "#4caf5099"; e.target.style.color = "#e0e0e0"; }}
          onBlur={(e) => { e.target.style.borderColor = "#252525"; e.target.style.color = "#555"; }}
        ...
        <button
          ...
          disabled={!custom.fondo?.trim()}
          style={{ padding: "6px 12px", background: custom.fondo?.trim() ? "#1a2a1a" : "#181818", border: `1.5px solid ${custom.fondo?.trim() ? "#4caf5066" : "#252525"}`, borderRadius: "8px", color: custom.fondo?.trim() ? "#4caf50" : "#444", fontFamily: "'DM Sans', sans-serif", fontWeight: "600", fontSize: "12px", cursor: custom.fondo?.trim() ? "pointer" : "not-allowed", whiteSpace: "nowrap", transition: "all 0.15s" }}
          onMouseOver={(e) => { if (custom.fondo?.trim()) { e.currentTarget.style.borderColor = "#4caf50"; e.currentTarget.style.background = "#1e3a1e"; }}}
          onMouseOut={(e) => { if (custom.fondo?.trim()) { e.currentTarget.style.borderColor = "#4caf5066"; e.currentTarget.style.background = "#1a2a1a"; }}}
        >+ Agregar</button>
      </div>
      {/* Máscara sub-section */}
      <div style={{ marginTop: "14px", background: "#111", borderRadius: "10px", border: "1px solid #222", padding: "12px 14px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "10px" }}>
          <span style={{ fontSize: "11px", color: "#F5C51888", fontWeight: "700", letterSpacing: "0.5px" }}>◈ MÁSCARA / MARCO</span>
          <span style={{ fontSize: "10px", color: "#333" }}>— evita bordes cuadrados o cortantes</span>
          {mascaraArr.length > 0 && (
            <button onClick={() => onToggle("mascara", "__clear__")}
              style={{ marginLeft: "auto", background: "none", border: "1px solid #333", borderRadius: "5px", color: "#555", fontSize: "10px", padding: "3px 7px", cursor: "pointer", fontFamily: "'DM Sans', sans-serif" }}
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: "7px" }}>
          {MASCARA_OPTIONS.map((m) => (
            <button key={m.value} onClick={() => onToggle("mascara", m.value)} title={m.desc}
              style={{
                padding: "6px 13px", borderRadius: "999px",
                border: `1.5px solid ${mascaraArr.includes(m.value) ? "#F5C518" : "#2a2a2a"}`,
                background: mascaraArr.includes(m.value) ? "#F5C51818" : "#181818",
                color: mascaraArr.includes(m.value) ? "#F5C518" : "#555",
                fontFamily: "'DM Sans', sans-serif", fontSize: "12px",
                fontWeight: mascaraArr.includes(m.value) ? "600" : "400",
                cursor: "pointer", transition: "all 0.18s", whiteSpace: "nowrap",
              }}
              onMouseOver={(e) => { if (!mascaraArr.includes(m.value)) { e.currentTarget.style.borderColor = "#444"; e.currentTarget.style.color = "#888"; }}}
              onMouseOut={(e) => { if (!mascaraArr.includes(m.value)) { e.currentTarget.style.borderColor = "#2a2a2a"; e.currentTarget.style.color = "#555"; }}}
            >{m.label}</button>
          ))}
        </div>
        {mascaraArr.length > 0 && (
          <p style={{ marginTop: "8px", fontSize: "11px", color: "#5a5a2a", fontStyle: "italic" }}>
            → {mascaraArr.map(v => MASCARA_OPTIONS.find(m => m.value === v)?.desc).filter(Boolean).join(" + ")}
          </p>
        )}
      </div>
    </div>
...
export default function App() {
  ...
  const [hmrCards, setHmrCards]         = useState(() => {
    ...
    HMR_CATS.forEach(cat => {
      const pool = HMR_BANK[cat];
      init[cat] = { value: pool[Math.floor(Math.random() * pool.length)], locked: false, active: true, generating: false, analyzingImg: false, imgRef: null, custom: "", showDropdown: false, showCustom: false };
    });
    return init;
  });
        ...
        <div style={{ display: "flex", gap: "16px", width: "100%", maxWidth: "1020px", alignItems: "flex-start" }}>
          ...
          <div style={{ flex: 1, background: "#161616", borderRadius: "14px", border: "1px solid #222", padding: "20px", display: "flex", flexDirection: "column" }}>
            ...
            <div style={{ borderTop: "1px solid #1e1e1e", paddingTop: "18px", display: "flex", flexDirection: "column", gap: "18px" }}>
              ...
              {/* Formato + Resolución + Formato salida */}
              {["formato"].map((cat, i) => (
                <div key={cat}>
                  <ModuleRow cat={cat} selected={selected} onToggle={toggle} custom={custom} onCustomChange={setCustomField} />
                  <div style={{ borderTop: "1px solid #1c1c1c", marginTop: "18px" }} />
                </div>
              ))}
              ...
              {/* Fondo + Máscara */}
              <FondoRow selected={selected} onToggle={toggle} custom={custom} onCustomChange={setCustomField} />
            </div>
